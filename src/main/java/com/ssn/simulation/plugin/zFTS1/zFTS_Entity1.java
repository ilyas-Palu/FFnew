...
public class zFTS_Entity1 extends Entity {
            ...
            if (!isMoving()) {
                if (!blockedTransfer) {
                    if (!DestinationWay1.isEmpty()) {
                        ...
                        if (vProcess.equals("POSO")) { // besser wieder ändern auf poso.subtype bzw. statisches Attribut
                        ...
                        if (!this.isAt(to) && !this.hasItem()) {
                            if (this.isAt(from)) {
                                ...
                                this.moveOutMach();
                                if (!this.isMoving()) {
                                    if (from.nextWaypoint(to.getWaypointCode()) != null) { // rendundante logik mit
                                                                                           // wtsk
                                                                                           // evtl
                                                                                           // auslagern
                                        from = from.nextWaypoint(to.getWaypointCode());
    ...
    public zFTS_Waypoint calcDest(Entity zDest) { // Berechne kürzeste Distanz //CN1 Achtung es sollen eig nicht alle
        ...
        for (zFTS_Waypoint element : allWaypoints) {
            ...
            if (element.getWaypointCode() == 61 || element.getWaypointCode() == 72) {
                if (comp1 < dist1) {
                    dist1 = comp1;
                    mindist = element;
                }
        ...
        if (mindist.getWaypointCode() == 72) { // Weitere Anpassung KTech
            for (zFTS_Waypoint element : allWaypoints) {
                if (element.getWaypointCode() == 71) {
                    mindist = element;
                }
            }
        }
    ...
    public void handleSrcTransfer(long timeEnd, String process) { // Allgemeine HU Transfer Methode auf FTF
        ...
        if (this.srcdest != null) {
            ...
            if (srcdest.hasItem()) {
                ...
                if (HU.getId().equals(wtorder.HU_Nummer) || this.controller.isIgnoreHuId()) {
                    // Ktech Einbindung ignorehuid
                    // cn1 korrekte HU ID
                    // Prüfung
    ...
    public Entity mapPaarbit(Entity destMach2, String codeEnding) {// korrektes Mapping cn1
        ...
        parts[2] = codeEnding; // Parameter Einbau evtl
        ...
        try {
            ...
            core.logInfo(this, "No matching " + codeEnding + " Conveyor found for " + destMach2.getId());
        }
        return null;
    ...
    public void infoTG(Item HU, String MFSError) {
        ...
        try {
            ...
            // Ktech Auslagerung jetzt im Connector
            if (MFSError != null) {
                switch (MFSError) {
                ...
                // Sicherheitsabfrage ignorHuId für Ktech, evtl obsolet
